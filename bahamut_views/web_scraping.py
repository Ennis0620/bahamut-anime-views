...
def plot(x, y):        #畫圖的參數調整
    plt.figure(figsize=(13, 4.5))
    plt.style.use("seaborn-deep")
    plt.bar(x, y)
    plt.title("views per episode diagram", fontsize = 25)
    plt.xlabel("episode", fontsize = 20)
    plt.ylabel("views", fontsize = 20)
    #plt.ylim([0,600000]) 如果要統一標準
...
def get_rank():
    ...
    tag_anime_name = tag_views.find_all("p", attrs = {"class", "anime-name_for-marquee"})  #找到全部動漫名稱的標籤
    tag_watch_number = tag_views.find_all("div", attrs = {"class", "anime-watch-number"})  #找到全部總觀看次數的標籤
...
def get_main_url():
    ...
    tag_views = soup.find("div", attrs = {"class", "newanime-wrap normal-ver newanime-content-hide"})
    tag_anime_name = tag_views.find_all("p", attrs = {"class", "anime-name_for-marquee"})  #找到全部動漫名稱的標籤
    tag_anime_url = tag_views.find_all("div", attrs = {"class", "anime-block"})            #找到全部動漫的影片網址
    anime_name_list = []
    for i in range(len(tag_anime_name)):
        anime_name = tag_anime_name[i]
        name = anime_name.text
...
def get_episode_view(index, main_url_list):
    ...
    if tag_episode == None:  #如果沒有歷史集數  代表是電影  或是此動漫目前只有一集 將不採計
        ...
        if len(episode_dict.keys()) == 0:
            ...
            plot(episode_number_list, episode_watch_number_list)  #填入對應的x.y座標並畫圖
    return True
...
test1 = get_rank()
test2 = get_main_url()
print(get_detail(0, test2))
get_episode_view(0, test2)
'''